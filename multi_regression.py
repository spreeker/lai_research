...
def calculate_ss(y, y_pred, measurements, plot_predictor_labels, parameters):
    ...
    log.info(parameters)
    ss_err = np.power(y - y_pred, 2).sum() / 120
    ss_tot = np.power(y, 2).sum() / 120
    ss_tot_p = np.power(y_pred, 2).sum() / 120
    ...
    for m, l, p in zip(measurements, plot_predictor_labels, parameters):
        ...
        # mss_tot = np.power(m, 2).sum() / 120
        mss_reg = np.power(m * p, 2).sum() / 120
        # mss_reg = np.power(mp, 2).sum() / 120
...
def make_local_plot(grid, lai, cru, timestamps, geotransform, projection):
    ...
    lai_at_location = normalize(lai_at_location)
...
def plot_model_map(
        green, grid_model_rmse, invalid, title='rmse', version=None):
    ...
    if version:
        geotransform, projection, bbox =  \
            create_lai_cube.extract_lai_meta_v006()
        x_size = 2400
        y_size = 2400
    else:
        geotransform, projection, bbox = create_lai_cube.extract_lai_meta()
...
def main_world(plotlocation=False):
    ...
    geotransform, projection, bbox = create_lai_cube.extract_lai_meta_v006()
    ...
    # print(len(locations_model_rmse))
    plot_model_map(
        green, locations_model_rmse, invalid,
        title='lowest rmse', version='006')


def main(debug=False, plotlocation=False):
...
if __name__ == '__main__':
    ...
    main(debug=args.debug, plotlocation=args.plotlocation)
